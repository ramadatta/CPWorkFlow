...
subset_clusterNum <- meta_df %>% 
  ...
  filter(Plasmid_Clusters_renamed %in% "PC2") %>% 
...
subset_clusterNum %>%
  ...
  geom_point(aes(color = Lab_Species,shape= factor(Hospital)),size=2) +
  ...
  labs(title = "Plasmid persistence of Plasmid Cluster1 (PC1) carrying blaKPC-2 gene",
...
subset_clusterNum %>%
  ...
  ggplot(aes(Date_of_culture, Unique_Plasmid_Cluster)) +
  geom_line(aes(group=Unique_Plasmid_Cluster), color='grey') +
  geom_point(aes(color = Lab_Species,shape= factor(Hospital)),size=2, alpha = 0.7,) +
  #scale_color_manual(palette = "Set3") +
  #scale_color_brewer(palette = "Paired") +
  #scale_fill_manual(values = colorRampPalette(brewer.pal(10, "Dark2"))(10)) +
...
# print(paste0("Total Isolate Count: ",total_isolatecount,"------Total Patient Count: ",total_patientcount))
# print(paste0("Total Clonal Cluster Count: ",cc_count,"------Total Patient Count: ",cc_count_totalPIDs))
# print(paste0("Total Isolates in Clonal Clusters ",cc_isolatecount))
# print(paste0("Total Isolates involved in HGT: ",noncc_backbone_isolatecount))
...
mycol_palette1 = c("darkorange","purple","cyan4",
                             "#e81123", "#ec008c","#68217a",
                             "#00188f", "#00bcf2", "black",
                             "maroon")
...
""
"blaNDM-1 acquired gene acquisition in 225 isolates"

CC_annotlabel <- glue::glue("<span style='font-size:10pt;font-family:Roboto Condensed;color:#000000'> Clonal Transmission: ",
                            "<span style='font-size:15pt;font-family:Roboto Condensed;color:#203864'>",total_isolates_CC,"</span>",
...
HGT_annotlabel <- glue::glue("<span style='font-size:10pt;font-family:Roboto Condensed;color:#000000'> Plasmid Transmission: ",
  "<span style='font-size:15pt;font-family:Roboto Condensed;color:#C55A11'>",total_isolates_HGT,"</span>", 
  "<span style='font-size:10pt;font-family:Roboto Condensed;color:#000000'> isolates (",
...
gg1 <- subset_clusterNum %>%
  ...
  geom_point(aes(color = Lab_Species,shape= factor(Hospital)),size=2, alpha = 0.7,) +
  scale_color_paletteer_d("yarrr::basel") +
  #scale_color_manual(values = mycol_palette1) +
  geom_text_repel(data = labelInfo2, aes(x = Date_of_culture, y = Unique_Plasmid_Cluster,
  ...
  labs(title = "Persistence of **Plasmid Cluster2** (PC2) carrying blaNDM-1 gene",
       #subtitle="of a facet barplot in ggplot",
      # tag = "Label Information\n\nSpecies; ST; Isolate Count;\nHospital Count",
       shape="Hospital", colour="Species",
       caption = "**Label Information:** Species; ST; Isolate Count; Hospital Count"
  ...
  annotate(geom = "richtext", x = as.Date("2012-02-01"), y = 200,  label = CC_annotlabel,fill = NA, family= "Roboto Condensed", label.color = NA) +
  annotate(geom = "richtext", x = as.Date("2012-02-01"), y = 190,  label = HGT_annotlabel,fill = NA, label.color = NA)
