...
# Changes from previous v1: removing epi information from the clonal cluster points, adding species, Final_ST, keeping hospital count and isolate counts

meta_df <- data.table::fread("/data02/Analysis/Projects/2_CPE_Transmission/Reference_Excels/Closed_CP_Plasmids_metadata_20230301.csv", header= TRUE, sep = ",")

meta_df <- meta_df %>% 
  mutate(ShortFinal_Species=case_when(stringr::str_detect(Final_Species,"Escherichia coli") ~ "EC", 
                                              stringr::str_detect(Final_Species,"Klebsiella pneumoniae" ) ~ "KP", 
                                              stringr::str_detect(Final_Species,"Enterobacter cloacae" ) ~ "ECL",
                                              stringr::str_detect(Final_Species,"Citrobacter amalonaticus" ) ~ "CA",
                                              stringr::str_detect(Final_Species,"Citrobacter farmeri" ) ~ "CF",
                                              stringr::str_detect(Final_Species,"Citrobacter freundii" ) ~ "CFR",
                                              stringr::str_detect(Final_Species,"Citrobacter rodentium" ) ~ "CR",
                                              stringr::str_detect(Final_Species,"Citrobacter koseri" ) ~ "CK",
                                              stringr::str_detect(Final_Species,"Enterobacter aerogenes" ) ~ "EA",
                                              stringr::str_detect(Final_Species,"Klebsiella oxytoca" ) ~ "KO",
                                              stringr::str_detect(Final_Species,"Morganella morganii" ) ~ "MM",
                                              stringr::str_detect(Final_Species,"Serratia marcescens" ) ~ "SM",
                                              TRUE ~ "Others"))
...
labelInfo <- subset_clusterNum %>%
  ...
  select(Unique_Plasmid_Cluster,Date_of_culture,ShortFinal_Species,Final_ST) %>%
  group_by(Unique_Plasmid_Cluster) %>% 
...
labelInfo2 <- left_join(labelInfo,cc_isolate_hospCount,by=c("Unique_Plasmid_Cluster"= "Clonal_cluster")) %>%
  mutate(CC_label = paste(ShortFinal_Species,Final_ST,IsolateCount_byCC,HospitalCount_byCC, sep = ";")) %>% 
...
gg1 <- subset_clusterNum %>%
  ...
  geom_point(aes(color = Final_Species,shape= factor(Hospital)),size=2, alpha = 0.5) +
  ...
  labs(
      ...
      # tag = "Label Information\n\nSpecies; Final_ST; Isolate Count;\nHospital Count",
       shape="Hospital", colour="Species",
       caption = "**Label Information:** Species; Final_ST; Isolate Count; Hospital Count"
  ) +
...
ggsave(filename = "PC1.jpeg", width = 11, height = 12, device='jpeg', dpi=300)
p2
#ggsave(filename = "PC2.jpeg", width = 12, height = 9, device='jpeg', dpi=700)
ggsave(filename = "PC2.jpeg", width = 11, height = 12, device='jpeg', dpi=300)
p3
#ggsave(filename = "PC3.jpeg", width = 10, height = 7, device='jpeg', dpi=700)
ggsave(filename = "PC3.jpeg", width = 9, height = 9, device='jpeg', dpi=300)
p4
#ggsave(filename = "PC4.jpeg", width = 10, height = 7, device='jpeg', dpi=700)
ggsave(filename = "PC4.jpeg", width = 9, height = 9, device='jpeg', dpi=300)
p5
#ggsave(filename = "PC5.jpeg", width = 10, height = 7, device='jpeg', dpi=700)
ggsave(filename = "PC5.jpeg", width = 10, height = 9, device='jpeg', dpi=300)
